...
func TestSchema(t *testing.T) {
	schemaBackup := "../pkg/ansible/schema/schema.json.bak"
	schemaPath := "../pkg/ansible/schema/schema.json"
	...
	output := &bytes.Buffer{}
	cmd := exec.Command("../bin/mixins/ansible/ansible", "schema")
	cmd.Stdout = output
	cmd.Stderr = output

	err = cmd.Start()
	require.NoError(t, err, "failed to start the ansible schema command")

	err = cmd.Wait()
	t.Log(output)
	require.NoError(t, err, "ansible schema failed")
}
